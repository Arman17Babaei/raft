...
class Node:
    ...
    def run(self):
        self.process = subprocess.Popen(self.cmd.split(), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

    def terminate(self):
        import os
        print(self.process.pid)
        os.kill(self.process.pid, 2)
        for pid in subprocess.check_output(f"lsof -t -i :{self.port}", shell=True).strip().split(b'\n'):
            try:
                [os.kill(int(pid), 2)]
            except:
                pass
